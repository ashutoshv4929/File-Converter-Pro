  ...
  const splitPDF = async (file) => {
    try {
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        const blob = new Blob([pdfBytes], { type: "application/pdf" });
        zipBlobs.push({ blob, filename: `page_${i + 1}.pdf` });
  ...
  const pdfToImages = async (file) => {
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        imageBlobs.push({ blob, filename: `page_${i}.jpg` });
              ...
              <div className="mt-4 p-4 bg-green-500/20 border border-green-500/30 rounded-lg">
                {result.text ? (
                  ...
                  <div>
                    ...
                    {result.downloadReady && result.blob && (
                      <div className="mt-4">
                        <button
                          onClick={() =>
                            downloadFile(result.blob, result.filename)
                          }
                          className="bg-gradient-to-r from-green-500 to-blue-500 text-white px-6 py-3 rounded-lg font-semibold hover:from-green-600 hover:to-blue-600 transition-all duration-300 flex items-center space-x-3 group relative overflow-hidden shadow-lg"
                        >
                          {/* Primary Icon */}
                          <span className="text-xl transition-all duration-300 group-hover:scale-110 group-hover:opacity-0">
                            📥
                          </span>
                          {/* Modern Icon Overlay */}
                          <span className="text-xl absolute left-6 opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-75 group-hover:scale-110">
                            💾
                          </span>
                          {/* Animated Background */}
                          ...
                          {/* Pulse Effect */}
                          <div className="absolute inset-0 bg-gradient-to-r from-white/10 to-transparent rounded-lg opacity-0 group-hover:opacity-100 animate-pulse"></div>
                        </button>
                        <p className="text-gray-400 text-xs mt-2">
                          Click to download the processed file to your device
                        </p>
                      </div>
                    )}

                    {/* Multiple files download (for split PDF and PDF to images) */}
                    {result.downloadReady &&
                      (result.zipBlobs || result.imageBlobs) && (
                        <div className="mt-4 space-y-2">
                          <p className="text-blue-300 font-semibold mb-3">
                            {result.zipBlobs
                              ? `${result.pages} PDF pages ready for download:`
                              : `${result.images} images ready for download:`}
                          </p>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                            {(result.zipBlobs || result.imageBlobs)?.map(
                              (item, index) => (
                                <button
                                  key={index}
                                  onClick={() =>
                                  ...
                                  className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 rounded text-sm hover:from-purple-600 hover:to-pink-600 transition-all duration-300 flex items-center space-x-2 group relative overflow-hidden"
                                >
                                  {/* Primary Icon */}
                                  <span className="text-sm transition-all duration-300 group-hover:scale-110 group-hover:opacity-0">
                                    📄
                                  </span>
                                  {/* Modern Icon Overlay */}
                                  <span className="text-sm absolute left-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-75 group-hover:scale-110">
                                    📥
                                  </span>
                                  {/* Animated Background */}
                                  <div className="absolute inset-0 bg-white/20 transform scale-0 group-hover:scale-100 transition-transform duration-300 rounded"></div>
                          ...
                          <button
                            onClick={() => {
                              (result.zipBlobs || result.imageBlobs)?.forEach(
                                (item) => {
                                  downloadFile(item.blob, item.filename);
                                }
                              );
                            }}
                            className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-3 rounded-lg font-semibold hover:from-orange-600 hover:to-red-600 transition-all duration-300 flex items-center justify-center space-x-3 group relative overflow-hidden shadow-lg mt-3"
                          >
                            {/* Primary Icon */}
                            <span className="text-xl transition-all duration-300 group-hover:scale-110 group-hover:opacity-0">
                              📦
                            ...
                            {/* Modern Icon Overlay */}
                            <span className="text-xl absolute opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-75 group-hover:scale-110">
                              🚀
                            </span>
                            {/* Animated Background */}
                            <div className="absolute inset-0 bg-white/20 transform scale-0 group-hover:scale-100 transition-transform duration-300 rounded-lg"></div>
                          ...
                          <p className="text-gray-400 text-xs mt-2 text-center">
                            Files will be downloaded individually to your device
                          </p>
                        </div>
                      )}

                    {/* Success message without auto-download info */}
                    {result.downloadReady && (
                      <div className="mt-4">
                        <div className="bg-green-500/20 border border-green-500/30 rounded-lg p-3">
                          ...
                          <p className="text-gray-300 text-sm">
                            Your file has been processed successfully. Click the
                            download button above to save it to your device.
                          </p>
                        </div>
                      </div>
                    )}
